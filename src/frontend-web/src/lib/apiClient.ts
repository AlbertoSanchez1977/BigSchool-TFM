...
// Respuesta con datos + metadatos de paginación (para endpoints que incluyen meta).
export interface WithMeta<T> {
  data: T | null
  meta: unknown
}

// Variante de request que, además del data, devuelve el meta del envelope.
// Necesaria para endpoints paginados donde la UI necesita totalCount/totalPages.
async function requestWithMeta<T>(
  ...
  let response: Response
  try {
    response = await fetch(`${options.baseUrl}${path}`, { method: 'GET', headers })
  } catch {
    throw new ApiError('NETWORK_ERROR', 'Error de red — el servidor no responde')
  }
...
export interface ApiClient {
  ...
  getWithMeta<T>(path: string): Promise<WithMeta<T>>
...
export function createApiClient(options: ApiClientOptions): ApiClient {
  return {
    ...
    getWithMeta: <T>(path: string) => requestWithMeta<T>(options, path),
