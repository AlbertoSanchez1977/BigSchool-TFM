...
- **Gráfica B** (nueva): `useMonthlySeries` — una llamada a `monthly` por cada `MainCategory` del tipo elegido (7 gasto / 4 ingreso, vía `CATEGORIES_BY_TYPE` derivado del enum), `monthly-stacked-bars.tsx` (barras apiladas por mes, paleta `--chart-1..4` cíclica).
