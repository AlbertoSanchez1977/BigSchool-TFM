...
import {
  ChevronLeft, Plus, StickyNote, Trash2, TrendingUp, TrendingDown, Minus,
} from 'lucide-react'
...
import type { HoldingListItem } from '@/types/portfolios'
...
function HoldingCard({
  ...
  const pnlClass     = pnlPositive ? 'text-positive' : 'text-negative'
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mb-3 grid grid-cols-3 gap-2 text-sm">
          ...
          <div>
            ...
            <p className={`flex items-center gap-0.5 font-medium tabular-nums ${pnlClass}`}>
              <PnLIcon className="h-3.5 w-3.5" />
              {holding.unrealizedPnL >= 0 ? '+' : ''}{formatAmount(holding.unrealizedPnL, currency)}
            </p>
          </div>
        </div>
        ...
        {!confirmDelete ? (
          <div className="flex gap-2">
            ...
            <Button
              ...
              data-testid="btn-delete-holding"
            >
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                type="button" variant="destructive" size="sm" className="flex-1"
      ...
      {/* Modal editar notas — montado solo cuando está abierto */}
      {editNotesOpen && (
        <EditNotesModal
      ...
      {portfolio && (
        <div className="mb-6 flex flex-wrap items-center gap-4 rounded-lg border border-border bg-card px-4 py-3">
          <div>
            ...
            <p className={`text-sm font-semibold tabular-nums ${portfolio.realizedPnL >= 0 ? 'text-positive' : 'text-negative'}`}>
              {portfolio.realizedPnL >= 0 ? '+' : ''}{formatAmount(portfolio.realizedPnL, currency)}
            </p>
          </div>
          <Minus className="h-3 w-3 text-muted-foreground/30 hidden sm:block" />
