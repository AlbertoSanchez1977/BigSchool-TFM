...
public class CreateSubCategoryCommandHandler : IRequestHandler<CreateSubCategoryCommand, SubCategoryFromCategoryDto>
{
    ...
    public async Task<SubCategoryFromCategoryDto> Handle(CreateSubCategoryCommand request, CancellationToken cancellationToken)
    {
        ...
        return new SubCategoryFromCategoryDto(sub.IdSubCategory, (int)sub.IdMainCategory, sub.IdMainCategory.ToString(), sub.Name);
    }
}
